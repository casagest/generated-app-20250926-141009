...
import { Bot, User, Send, Wrench, Trash2 } from 'lucide-react';
...
const initialWelcomeMessage = {
  ...
  content: "Hello! I'm the AI assistant for Aura Dental. I can help qualify you as a new patient. To start, could you please tell me your full name?",
  timestamp: Date.now(),
};
export function ChatbotPage() {
  ...
  const loadCurrentSession = useCallback(async () => {
    ...
    if (response.success && response.data) {
      // If the session is empty, start with the welcome message
      const messages = response.data.messages.length > 0 ? response.data.messages : [initialWelcomeMessage];
      setChatState(prev => ({ ...prev, ...response.data, messages, sessionId: chatService.getSessionId() }));
    }
  }, []);
  useEffect(() => {
  ...
  const handleClear = async () => {
    ...
    setChatState(prev => ({ ...prev, messages: [initialWelcomeMessage], streamingMessage: '' }));
  };
  return (
